(defpackage :postmodern-async-tests
  (:use :common-lisp :Eos :postmodern-async :simple-date))

(in-package :postmodern-async-tests)
...
(def-suite :postmodern-async)
(in-suite :postmodern-async)
...
(test ensure-transaction
  (with-test-connection
    (with-transaction ()
      (ensure-transaction
        (is (eql postmodern-async::*transaction-level* 1))))
    (is (eql postmodern-async::*transaction-level* 0))
    (ensure-transaction
      (is (eql postmodern-async::*transaction-level* 1)))))
...
(test notification
  (with-test-connection
    ...
    (with-test-connection
      (execute (:notify 'foo)))
    (is (cl-postgres-async:wait-for-notification *database*) "foo")))
