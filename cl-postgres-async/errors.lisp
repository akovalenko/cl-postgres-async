...
extract the constraint name."
  (labels ((extract-quoted-part (string n)
    ...
    (let ((message (database-error-message err)))
      (typecase err
        (cl-postgres-async-error:not-null-violation (extract-quoted-part message 0))
        (cl-postgres-async-error:unique-violation (extract-quoted-part message 0))
        (cl-postgres-async-error:foreign-key-violation (extract-quoted-part message 1))
        (cl-postgres-async-error:check-violation (extract-quoted-part message 1))))))
...
(in-package :cl-postgres-async-error)
