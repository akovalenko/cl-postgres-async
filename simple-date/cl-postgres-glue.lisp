...
(defconstant +apomo-postgres-day-offset+ -60)
(defconstant +apomo-usecs-in-one-day+ (* 1000 1000 3600 24))

(flet ((interpret-timestamp (usecs)
         (multiple-value-bind (days usecs) (floor usecs +apomo-usecs-in-one-day+)
           (make-instance 'timestamp :days (+ days +apomo-postgres-day-offset+)
                          :ms (floor usecs 1000)))))
  (cl-postgres:set-sql-datetime-readers
   :date (lambda (days)
           (make-instance 'date :days (+ days +apomo-postgres-day-offset+)))
   ...
   :interval (lambda (months days usecs)
               (make-instance 'interval :months months
                              :ms (floor (+ (* days +apomo-usecs-in-one-day+) usecs) 1000)))))

(defmethod cl-postgres-async:to-sql-string ((arg date))
  (multiple-value-bind (year month day) (decode-date arg)
    (values (format nil "~4,'0d-~2,'0d-~2,'0d" year month day) "date")))

(defmethod cl-postgres-async:to-sql-string ((arg timestamp))
...
(defmethod cl-postgres-async:to-sql-string ((arg interval))
